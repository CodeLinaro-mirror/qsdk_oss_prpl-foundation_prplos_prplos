...
Check that untrusted service cannot read the data from USP bus when RequiredRoles is "Untrusted":
  ...
  $ R "${S} && install_ctr --url ${READER_SERVICE_URL} --password --username --ee --uuid --privileged true --usprequired \"Untrusted\" --uspregisterpaths \"Device.TrustedReader.\" --uspautomountipc \"USP_UDS_Authenticated\"" > /dev/null
  $ sleep ${SLEEP_TIME}
...
Check that untrusted service cannot register model on USP bus when RegisterTrustPaths is empty:
  ...
  $ R "${S} && install_ctr --url ${PROVIDER_SERVICE_URL} --password --username --ee --uuid --privileged true --usprequired \"Full Access\" --uspautomountipc \"USP_UDS_Authenticated\"" > /dev/null
  $ sleep ${SLEEP_TIME}
  $ R "${S} && get_container_info --uuid"
  Active
  latest
...
Update the RegisterTrustPaths parameter for service and check that untrusted service can register only allowed paths on USP bus:
  ...
  $ R "${S} && update_ctr --url ${PROVIDER_SERVICE_URL} --password --username --ee --uuid --privileged true --usprequired \"Full Access\" --uspregisterpaths \"Device.TrustedProvider1.\" --uspautomountipc \"USP_UDS_Authenticated\"" > /dev/null
  $ sleep ${SLEEP_TIME}
  $ R "usp-cli -lj 'Device.USPServices.Trust.[EndpointID==\"${CTR_ENDPOINTID}\"].?' | sed '/^$/d'"
