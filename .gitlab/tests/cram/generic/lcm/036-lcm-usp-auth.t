...
Install the container and check its status and type:
  ...
  $ R "${S} && install_ctr --url ${SERVICE_URL} --password --username --ee --uuid --privileged true --usprequired \"Full Access\"" > /dev/null
  $ sleep ${SLEEP_TIME}
...
Check that the container has the expected EndpointID, AutoMountIPC value:
  ...
  USP_UDS_Unauthenticated
  $ R "ba-cli -l 'Device.SoftwareModules.ExecutionUnit.[EUID==\"$CTR_ID\"].X_PRPLWARE-COM_RegisterTrustPaths?' | sed '/^$/d'"
...
Install the container as untrusted and check its status and type:
  ...
  $ R "${S} && install_ctr --url ${SERVICE_URL} --password --username --ee --uuid --privileged true --usprequired \"Full Access\" --uspregisterpaths \"Device.LCMSampleApp.\" --uspautomountipc \"USP_UDS_Authenticated\"" > /dev/null
  $ sleep ${SLEEP_TIME}
  $ R "${S} && get_container_info --uuid"
  Active
  latest
...
Check that trusted service can be updated to untrusted:
  ...
  $ R "ba-cli -l 'Cthulhu.Container.Instances.[ContainerId==\"${CTR_ID}\"].AutoMountIPC?' | sed '/^$/d'"
  USP_UDS_Unauthenticated
  $ R "${S} && update_ctr --url ${SERVICE_URL} --password --username --ee --uuid --privileged true --usprequired \"Full Access\" --uspregisterpaths \"Device.LCMSampleApp.\" --uspautomountipc \"USP_UDS_Authenticated\"" > /dev/null
  $ sleep ${SLEEP_TIME}
  $ R "${S} && get_container_info --uuid"
  Active
  latest
